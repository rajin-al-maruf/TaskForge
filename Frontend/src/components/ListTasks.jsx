...
const ListTasks = ({ listName, tasks, isLoading, error, onOpenModal, onEditTask, onToggleComplete, onDeleteTask, filters, setFilters, sortBy, setSortBy }) => {
  const hasActiveFilters = filters && (filters.priority !== 'all' || filters.status !== 'all');
        ...
        <div className='flex flex-col gap-4 mb-6 sm:flex-row sm:items-center sm:justify-between'>
          ...
          <div className='inline-flex items-center bg-neutral-950/30 border border-neutral-800/60 rounded-xl shadow-sm'>
            <div className='flex items-center px-4 py-2 border-r border-neutral-800/60 rounded-l-xl bg-neutral-900/20'>
              <h1 className='text-white text-lg font-semibold tracking-wide capitalize'>{listName}</h1>
            </div>
      ...
      <div className='mt-6 space-y-3'>
        {isLoading ? (
          ...
          <EmptyTaskState 
            ...
            description={`You don't have any tasks in "${listName}" right now. Keep it up, or add a new task to get started.`} 
            onAdd={() => onOpenModal()}
          />
        ) : (
          tasks.map((task) => (
